//  Muliplication {a}•{b} in GF(2^8) galoisMultiply
function dot(a, b) {
  // convert inputs from  string to binary "11110000" => 11110000b
  a = parseInt(a, 2);
  b = parseInt(b, 2);
  let p = 0;
  let counter;
  ...
  const res = p.toString(2).slice(-8);
  return res;
}
...
const mixColumns = (B) => {
  const A = [
    ...
    return e.map((ee) => {
      return ee.toString(2).padStart(8, "0");
    });
  });
  const C = [];
  for (let i = 0; i < 4; i++) {
...
//export { aesEncrypt, aesDecrypt, sBox, invSBox, rCon };
